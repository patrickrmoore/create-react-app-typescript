...
module.exports = function(
  ...
  console.log(
    `Installing ${coreDependencies.join(", ")} as dependencies ${command}...`
  );
  console.log();

  const coreProc = spawn.sync(command, args.concat(coreDependencies), {
    stdio: "inherit"
  });
  if (coreProc.status !== 0) {
    console.error(
      `\`${command} ${args.concat(coreDependencies).join(" ")}\` failed`
    );
    return;
  }
